...
| client metadata와 암호화된 client/preshared key | MariaDB `waygate_clients` | 없음 | 활성 server 내 `name`, `tunnel_ip` unique constraint를 지키며 soft-delete 시 슬롯과 private/preshared ciphertext를 비우고 peer를 비활성화한다. |
...
Post-0.1.4 feature snapshot은 기존 release tag를 덮어쓰지 않고 CI에서 발행한 commit별 image digest로 배포한다. package version만으로 배포 소스를 구분할 수 없으므로 source commit, API/worker digest, Kolla wheel checksum과 이전 설정/DB backup을 함께 보존한다. Migration 002는 새 container 시작 전에 적용하는 additive schema 변경이며 `kolla-ansible deploy --tags waygate`의 bootstrap이 적용한다. `reconfigure`는 migration을 실행하지 않는다. Afterglow operator 환경은 release tag로 승격된 role만 설치하므로, 새 tag가 승격되기 전에는 설치된 0.1.4 role이 `agent_install_mode`를 렌더하지 않고 서비스 기본값 `cloud-init`이 적용된다.
...
{
  "schema_version": 1,
  "source_sha256": "d3cc9f3cfca8fb1c026d67ca4780470cc648bb7a744589cbcc5e390721c9a74f",
  "reviewed_at": "2026-09-26T14:39:04Z",
  "summary": "Reviewed staged Redis shutdown/cutover, trusted-proxy/rate-limit normalization, strict cloud deletion and parent-locked terminal/client cleanup, regression tests and code map. No schema, dependency, CI or process topology change. API/worker amd64 and arm64 builds and real MariaDB/Redis runtime smoke passed; service serial/xdist each 377 passed and 1 live skip; SDK 22 passed; both Ruff checks passed."
}
```
